...
export class OffreUpdateComponent implements OnInit {
  ...
  setFileData(event: Event, field: string, isImage: boolean): void {
    this.dataUtils.loadFileToForm(event, this.editForm, field, isImage).subscribe({
      error: (err: FileLoadError) =>
        this.eventManager.broadcast(new EventWithContent<AlertError>('jopGalSenApp.error', { ...err, key: `error.file.${err.key}` })),
    });
  }
  ...
  protected loadRelationshipsOptions(): void {
    ...
    this.posteService
      ...
      .pipe(map((postes: IPoste[]) => this.posteService.addPosteToCollectionIfMissing<IPoste>(postes, ...(this.offre?.postes ?? []))))
