...
type RestOf<T extends IOffre | NewOffre> = Omit<T, 'dateDePostule' | 'dateDeFin'> & {
  dateDePostule?: string | null;
...
export class OffreService {
  ...
  protected convertDateFromClient<T extends IOffre | NewOffre | PartialUpdateOffre>(offre: T): RestOf<T> {
    return {
      ...offre,
      dateDePostule: offre.dateDePostule?.format(DATE_FORMAT) ?? null,
      dateDeFin: offre.dateDeFin?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restOffre: RestOffre): IOffre {
    return {
      ...restOffre,
      dateDePostule: restOffre.dateDePostule ? dayjs(restOffre.dateDePostule) : undefined,
      dateDeFin: restOffre.dateDeFin ? dayjs(restOffre.dateDeFin) : undefined,
    };
  }
