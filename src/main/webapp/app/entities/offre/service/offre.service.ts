...
type RestOf<T extends IOffre | NewOffre> = Omit<T, 'postedDate'> & {
...
export class OffreService {
  ...
  protected convertDateFromClient<T extends IOffre | NewOffre | PartialUpdateOffre>(offre: T): RestOf<T> {
    return {
      ...offre,
      postedDate: offre.postedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restOffre: RestOffre): IOffre {
    return {
      ...restOffre,
      postedDate: restOffre.postedDate ? dayjs(restOffre.postedDate) : undefined,
    };
  }
